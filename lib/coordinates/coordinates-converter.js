...
export class CoordinatesConverter {
   ...
   * @param {number} localSiderealTime - local sidereal time in degrees
   * @param {number} flattening
   * @param {number} horizontalParallaxConstant
  ...
  static equatorialSphericalGeocentricToEquatorialSphericalTopocentric(
    ...
    const localSiderealTimeInRadians =
      MathHelper.degreesToRadians(localSiderealTime);

    const u = Math.atan((1 - flattening) * Math.tan(observerLatitudeInRadians));
    const rhoSinPhiPrime =
      (1 - flattening) * Math.sin(u) +
    ...
    const equatorialHorizontalParallaxInRadians = Math.asin(
      horizontalParallaxConstant / equatorialSphericalCoordinates.delta,
    );
    const localHourAngleInRadians =
      localSiderealTimeInRadians - rightAscensionInRadians;
   ...
   */
  static equatorialSphericalTopocentricToHorizontalSphericalCoordinates(
